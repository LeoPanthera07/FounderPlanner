...
export const DayPage = () => {
  ...
  const { dayData, loading, loadDay, updateField, updateFocusItem,
    toggleRoutine, updateRoutine, updateReflection,
    updateScheduleSlot, addShutdownItem, updateShutdownItem, removeShutdownItem } = useDayStore();
  ...
  const completedRoutines = dayData.routines.filter(r => r.done).length;
  const totalRoutines = dayData.routines.length;
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 flex flex-col gap-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">Daily Command · Routines · Reflection</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="text-xs text-slate-500">Routines: <span className="font-bold text-green-600">{completedRoutines}/{totalRoutines}</span></div>
          <div className="w-24 h-2 bg-slate-200 rounded-full overflow-hidden">
            <div className="h-full bg-green-500 rounded-full transition-all" style={{ width: `${(completedRoutines/totalRoutines)*100}%` }} />
          </div>
        </div>
      </div>
      ...
      <DailyRoutineChecklist routines={dayData.routines} onToggle={toggleRoutine} onUpdate={updateRoutine} />
      <HourlySchedule schedule={dayData.schedule} onUpdate={updateScheduleSlot} />
